...
{
	// User Defaults
	char Username[40] = STD_USERNAME;
	uint32_t AppId = 0;
	uint32_t AppBuildId = 0;
	uint64_t UserID = STD_USERID;

	// Directories and language default
	char SaveDirectory[0x400] = { 0 };
	char UGCDirectotry[0x400] = { 0 };
	char Language[40] = STD_LANGUAGE;
...
{
#ifdef _WIN64
	CHAR ClientDir[MAX_PATH] = { "clienthooked64.dll" };
	WCHAR WClientDir[MAX_PATH] = { L"clienthooked64.dll" };
#else
	CHAR ClientDir[MAX_PATH] = { "clienthooked.dll" };
	WCHAR WClientDir[MAX_PATH] = { L"clienthooked.dll" };
#endif
...
namespace ColdAPI_DLC
{
	...
	{
		// Check if the requested AppId is not 0 or the same as the default.
...
namespace ColdAPI_Storage
{
	...
	int32_t CFileWriteStreamClose(uint64_t hStream)
	{
		...
		if (!StreamRequested->second.Buffer || !StreamRequested->second.FileNameS) {
			return 2;
		}
...
namespace ColdAPI_General
{
	...
	void SetOverlayNotification(int Value)
	{
		...
		if (SetNotificationPositionExt)
			((void(*)(int32_t))SetNotificationPositionExt)(Value);	// Call the virtual loaded function, most people use typedef method, this is faster
		else
			return;
	}
	bool IsOverlayNeededOrEnabled()
	{
		...
		if (OverLayEn)
			return ((bool(*)())OverLayEn)(); // Call the virtual loaded function, most people use typedef method, this is faster
		return false;
	}
	...
	void* ProvideInterface(int type, const char* Version, bool ScanForName)
	{
		if (ScanForName)
		{
			...
			// Looks like the interface is not available, warn if the user wants it.
			if (Steam_Config::InterfaceNFound)
			{
				// Check if the user choose to ignore the current interface.
				bool Ignore = false;
	...
	void* ContextInitialization(void* Input)
	{
	...
	bool ReadIni()
	{
		...
		// Read from INI
		GetPrivateProfileStringA("SteamProfile", "Username", Steam_Config::Username, Steam_Config::Username, sizeof(Steam_Config::Username), SteamINI);
		GetPrivateProfileStringA("SteamProfile", "Language", Steam_Config::Language, Steam_Config::Language, sizeof(Steam_Config::Language), SteamINI);
		GetPrivateProfileStringA("SteamData", "SavePath", "Auto", Steam_Config::SaveDirectory, sizeof(Steam_Config::SaveDirectory), SteamINI);
		GetPrivateProfileStringA("SteamProfile", "UserID", _ui64toa(Steam_Config::UserID, UserIdStr, 10), UserIdStr, 20, SteamINI);
		Steam_Config::UserID = strtoull(UserIdStr, NULL, 10);

		Steam_Config::AppId						= GetPrivateProfileIntA("SteamData",			"AppID",							0,	SteamINI);
		Steam_Config::AppBuildId			= GetPrivateProfileIntA("SteamData",			"AppBuildID",					0,	SteamINI);
		Steam_Config::OnlineMod				= GetPrivateProfileIntA("SteamData",			"OnlineMode",					0,	SteamINI) != FALSE;
		Steam_Config::UnlockAllDLCS		= GetPrivateProfileIntA("SteamData",			"UnlockAllDLCs",			1,	SteamINI) != FALSE;
		Steam_Config::RemoteStorage		= GetPrivateProfileIntA("SteamData",			"SteamFileSave",			0,	SteamINI) != FALSE;
		Steam_Config::LowViolence			= GetPrivateProfileIntA("SteamProfile",		"LowViolence",				0,	SteamINI) != FALSE;
		Steam_Config::StubBypass			= GetPrivateProfileIntA("SteamAdditional","PatchSteamStub",			0,	SteamINI) != FALSE;
		Steam_Config::ClientEmulation = GetPrivateProfileIntA("SteamAdditional","ClientEmulation",		0,	SteamINI) != FALSE;
		Steam_Config::InterfaceNFound = GetPrivateProfileIntA("SteamAdditional","WarnInterfaceNFound",0,	SteamINI) != FALSE;
		Steam_Config::HookForInjection= GetPrivateProfileIntA("SteamClient",		"HookInjectionMode",	0,	SteamINI) != FALSE;
		...
		if (GetPrivateProfileIntA("SteamAdditional", "InterfaceChecker", NULL, SteamINI) == TRUE)
		{
			...
			if (ExpectedSFound)
			{
				// We now have the original steam_api section base address. Let's search the string in it.
...
namespace ColdAPI_InitAndShutDown
{
	...
	bool ColdClientInitialization()
	{
		if (!IsInitialized)
		{
			...
			if (Steam_Config::RemoteStorage)
			{
				ColdAPI_Storage::Storage_Initialization();
			}
